...
def train():
    ...
    hparams = config
    print("starting train", config)
    batch_size = 2**hparams["log2_batch_size"]
    # config
    ...
    for epoch in range(hparams["n_epochs"]):
        ...
        for batch_idx, (x, _) in enumerate(train_loader):
            x = x.view(batch_size, hparams["x_dim"])
            x = x.to(DEVICE)
        ...
        log.info(
            f"Epoch {epoch+1} complete! Average Loss: {avg_loss}"
        )
        print("")
    ...
    # Generate samples
    with torch.no_grad():
        noise = torch.randn(batch_size, hparams["latent_dim"]).to(DEVICE)
        generated_images = decoder(noise)

    save_image(
        generated_images.view(batch_size, 1, 28, 28), "generated_sample.png"
    )
